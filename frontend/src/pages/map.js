...
export default function Map() {
    ...
    useEffect(() => {
        ...
        setTimeout(() => {
            ...
            if (
                fetchedRoutePath &&
                fetchedRoutePath != undefined &&
                fetchedRoutePath != ""
            ) {
                setCurrentRoute(JSON.parse(fetchedRoutePath));
                setCurrentRouteOverview(fetchedRouteOverview);
                setWaypointsNum(
                    JSON.parse(fetchedRoutePath)["waypoints"].length > 1
                        ? JSON.parse(fetchedRoutePath)["waypoints"].length
                        : 2
                );
                fromRef.current.value = JSON.parse(fetchedRoutePath)["origin"];
                const directionsOverview = document.querySelector(
                    "#directionsOverview"
                );
                ...
                transportModeMenu.value =
                    JSON.parse(fetchedRoutePath)["travelMode"];
                const optimizeRoute =
                    document.querySelector("#optimizeRouteRef");

                optimizeRoute.checked =
                    JSON.parse(fetchedRoutePath)["optimizeWaypoints"];

                if (JSON.parse(fetchedRoutePath)["waypoints"].length > 1) {
                    //origin is in waypoints
                    firstToInput.value =
                        JSON.parse(fetchedRoutePath)["waypoints"][1];
                    setWaypointValues(
                        JSON.parse(fetchedRoutePath)["waypoints"].slice(1)
                    );
                } else {
                    //origin is not in waypoints
                    firstToInput.value =
                        JSON.parse(fetchedRoutePath)["waypoints"][0];
                    setWaypointValues(
                        JSON.parse(fetchedRoutePath)["waypoints"]
                    );
                }
    ...
    const addWaypoint = (e) => {
        ...
        } else if (waypointsNum === 10) {
            alert(
                "Requests using more than 10 waypoints are billed at a higher rate"
            );
        }
    ...
    const createMarker = (details) => {
        ...
        // Opens a google search for the amenity
    ...
    return (
        <>
            ...
            <DirectionsPopup
                closemodal={setOpenDirectionsModal}
                isOpen={openDirectionsModal}
            />
            <div className="bg-eggshell w-screen h-screen flex justify-between relative">
                ...
                <div className="bg-gray z-99 p-2 rounded-xl m-4 place-self-start fixed">
                    <button onClick={() => setShowSideBar(true)}>
                        <KeyboardDoubleArrowRightIcon
                            className={`${
                                showSidebar ? "hidden" : ""
                            } text-eggshell text-3xl cursor-pointer`}
                        />
                    </button>
                </div>
                <div
                    className={`${
                        showSidebar ? "" : "hidden"
                    } bg-gray z-99 h-screen w-1/4 place-self-start fixed`}
                >
                    <div className="mx-3 h-full flex flex-col">
                        <div className="basis-1/12 flex items-baseline justify-between">
                            {" "}
                            {/* title */}
                            <Link href="/">
                                <h1 className="font-titleFont font-bold text-eggshell text-2xl mt-4">
                                    Routourist
                                </h1>
                            </Link>
                        ...
                        <form className="basis-7/12 mt-2 flex flex-col justify-evenly">
                            <div
                                id="waypointsList"
                                className="font-bodyfont flex flex-col h-24 gap-2 overflow-auto"
                            >
                                {" "}
                                ...
                                <div className="flex">
                                    <input
                                        ...
                                        className=" px-3 py-1 border-1 w-11/12 rounded-md"
                                        type="text"
                                    ></input>
                                </div>
                                <div className="flex">
                                    <input
                                        ...
                                        className="toRef px-3 py-1 border-1 w-11/12 rounded-md"
                                        type="text"
                                    ></input>
                            ...
                            <div className="flex justify-center">
                                ...
                                <div className="flex items-center justify-between w-full mb-2">
                                    <button
                                        onClick={(e) => addWaypoint(e)}
                                        className="text-eggshell underline cursor-pointer hover:text-green"
                                    >
                                        Add Waypoint
                                    </button>
                                    <button
                                        onClick={(e) => resetWaypoints(e)}
                                        className="text-eggshell underline cursor-pointer hover:text-green"
                                    >
                                        Reset
                                    </button>
                            ...
                            <div className="font-bodyfont">
                                <select
                                    ...
                                    name="ModeTransport"
                                    className="bg-gray text-eggshell w-11/12 font-bodyfont border-2 border-eggshell rounded-md px-3 py-1 mb-2"
                                >
                            ...
                            <div>
                                ...
                                <label
                                    for="OptimiseChoice"
                                    className="text-eggshell font-bodyfont ml-3"
                                >
                                    Optimise Route
                                </label>
                                <p className="text-eggshell font-bodyfont text-xs">
                                    You can reduce your carbon footprint <br />{" "}
                                    by optmising your route!
                                </p>
                            </div>
                            ...
                            <div className="flex place-content-center mt-4">
                                ...
                                <div>
                                    <button
                                        onClick={(e) => calcRoute(e)}
                                        className="font-bodyfont w-full max-h-fit bg-green hover:bg-lightgreen py-2 px-3 rounded-lg drop-shadow-2xl mb-3"
                                    >
                                        Create Route
                                    </button>
                                    <button
                                        onClick={(e) => saveRoute(e)}
                                        className="font-bodyfont w-full max-h-fit bg-eggshell hover:bg-white py-2 px-3 rounded-lg drop-shadow-2xl"
                                    >
                                        Save Route
                                    </button>
                                </div>
                        ...
                        <div className="h-full font-bodyfont basis-3/12 flex items-center ">
                            {" "}
                            ...
                            <div className="h-fit">
                                ...
                                ></div>
                                <p
                                    id="directionsOverview"
                                    className="text-xs text-eggshell"
                                ></p>
                                {currentRoute.origin ? (
                                    <button
                                        onClick={() =>
                                        ...
                                        className="text-xs underline text-eggshell cursor-pointer"
                                    >
                                        Show Directions
                                    </button>
                                ) : (
                        ...
                        <div className="basis-1/12 flex place-content-center">
                            {" "}
                            {/* my saved routes button */}
                            <Link href={user ? "/savedroutes" : "/login"}>
                                <button className="font-bodyfont w-fit h-fit px-10 py-2.5 mb-4 bg-eggshell hover:bg-white rounded-lg drop-shadow-2xl">
                                    My Saved Routes
                                </button>
                            </Link>
                        </div>
                ...
                <div className="font-bodyfont z-99 fixed right-0 mt-4 mr-12 flex items-start h-14">
                    <div className="w-64 flex flex-col justify-center items-center">
                        <button
                            onClick={(e) => handleAttractionsDropdown(e)}
                            className="w-full max-h-fit bg-gray hover:bg-green rounded-xl text-eggshell py-1.5 px-4"
                        >
                            Show Nearby Attractions
                        </button>
                        <div
                            className={`${
                                isAttractionsDropdownOpen ? "" : "hidden"
                            } w-full bg-gray py-1.5 px-4 mt-2 rounded-xl opacity-80 text-eggshell`}
                        >
                    ...
                    {!user ? (
                        <Link href="/login">
                            <button className="max-w-fit bg-gray rounded-xl text-eggshell py-1.5 px-4 mx-3 hover:bg-green">
                        ...
                        <button
                            ...
                            className="max-w-fit bg-gray rounded-xl text-eggshell py-1.5 px-4 mx-3 hover:bg-green"
                        >
                            <LogoutIcon className="mr-1" />
                            Logout
                        </button>
                    )}
                    <button onClick={(e) => inforender(e)}>
